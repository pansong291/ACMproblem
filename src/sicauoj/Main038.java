...
/*
title:
校园内的路灯       -Time Limit Exceeded

description:
每条路一到天黑就开灯，一开就是一整夜，而电费按道路每米一天要花费1块钱。为了省钱要关掉一些街道的路灯。
为了确保教师和学生仍然感到安全，在夜晚一些街道变暗之后，在学校的每一个路口，
仍然会有至少一条照明的路径到其它任何一个路口。在不让川牛们感到不安全的情况下，学校可以节省的最大每日金额是多少?

input:
输入包含若干测试用例。每个测试用例分别以两个数字m=[1,200000]和n=[m-1,200000]开始，
分别是学校内路口的数量和道路数量，当m=n=0时终止。接下来有n个整数三元组x=[0,m),y=[0,m),z
指定路口x和路口y之间有一条长度为z米的双向道路，且x!=y。
每个测试用例保证了任意两个路口之间一定有一条可达的路线。每个测试用例中所有道路的总长度小于231231。

output:
每一个测试案例需要输出一行，表示学校每日可以节约的最高金额。
