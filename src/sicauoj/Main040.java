...
description:
现在有一串合法的括号，川牛将依次从左往右拿走一个左括号，与此同时你需要拿走一个右括号，
每次拿走之后剩下的需保证括号依然是合法的，直到取完所有括号，那么请问你有多少种不同的拿取方法呢？
（拿走不一样的括号即算一种）

input:
第一行将输入一个整数N=(0,300]。随后将有N行测试用例，每一行都将给出一串合法的括号序列S，S的长度l=[2,200]。

output:
输出每个测试用例一共有多少种拿法MOD 1000000007 的结果，各占一行。
