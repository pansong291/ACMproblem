...
description:
运输公司规定每个司机每天最多只能工作10小时，然后在指定的一家签定了免费入住协议的连锁酒店休息一晚。
运输公司也要求司机用最少的天数将货物交付。

input:
输入包含多个测试用例。每个测试用例的第一行是一个整数n=[2,10000]，表示在规划路线时要考虑的城市数。
城市的编号从1到 n, 其中1是起始城市, n 是目的地城市。
下一行包含一个整数 h=[0,min(n,100)]，后跟数字 c1,c2,...,ch。表示连锁酒店所在城市的编号。
第三行包含一个整数 m=[1,105]，为规划路线考虑的路的数量。接下来的m行描述路。
每条路包含3个整数 a=[1,n]、b=[1,n]、t=[1,600]，其中 a、b 是路连接的两个城市。
t是在这条路上行驶所需的时间。输入n=0时终止。

output:
对于每个测试用例，输出一行包含运输公司将货从城市1送到城市n而必须为司机预定入住的最少的酒店数量。
如果不可能找到路线使得驾驶员每天最多行驶10小时 ，则输出-1。
