 ...
 * tm: 鑫苑餐厅     --finish
 * 
 * ms: 会员们请川牛做一个长方形的大蛋糕，但不知道这块大的长方形蛋糕是否能在圆圆的桌面上放得下。
 * 
 * sr: 输入包含多个测试用例。每个测试用例占一行，以一个整数r=[1,1000]开始，即会员们所坐的圆桌表面的半径。
 * s   当r=0时结束。接下来的2个整数W=[1,L]和L=[W,1000]指定长方形蛋糕的宽度和长度。
 * 
 * sc: 每个测试用例输出蛋糕是否适合桌面。一个只触摸桌子边缘而不交叉的蛋糕也被认为适合的。
 * 
 * ei: 38 40 60
 *     32 50 70
 *     50 60 80
